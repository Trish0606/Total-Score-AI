import streamlit as st
from src.ingestion import fetch_live_scores

# 1. PAGE CONFIG & CONTROLS (Main Body - Outside Fragment)
st.set_page_config(page_title="Math AI Engine", page_icon="🏀", layout="wide")
st.title("🏀 Total Score AI Platform Arbitrage Tracker")

# These widgets are here so they can correctly control the state
st.sidebar.header("🕹️ Control Room")
league_choice = st.sidebar.selectbox("Select Target League", ["nba", "wnba"])
refresh_rate = st.sidebar.slider("Auto-Refresh Rate (seconds)", 10, 60, 30)

# 2. DATA RENDERING FRAGMENT
@st.fragment(run_every=refresh_rate)
def render_live_dashboard():
    # League Constants
    if league_choice == "nba":
        pts_per_min, mirror_time = 5.0, "6:00"
    else:
        pts_per_min, mirror_time = 4.0, "5:00"

    live_games = fetch_live_scores(league_choice)
    if not live_games:
        st.info("✨ No live games detected. Waiting for tip-off...")
        return

    for game in live_games:
        # Data Logic
        comp = game.get("competitions", [{}])[0]
        status = comp.get("status", {})
        if status.get("type", {}).get("state") != "in": continue
        
        t1_q = [int(q.get("value", 0)) for q in comp["competitors"][0].get("linescores", [])]
        t2_q = [int(q.get("value", 0)) for q in comp["competitors"][1].get("linescores", [])]
        
        q_total = (t1_q[status.get("period", 1)-1] + t2_q[status.get("period", 1)-1])
        game_total = sum(t1_q) + sum(t2_q)
        
        mirror_pred = q_total * 2
        fh_proj = (sum(t1_q[:2]) + sum(t2_q[:2])) if len(t1_q) >= 2 else (q_total * 2)

        # UI Display
        st.write("---")
        st.markdown(f"### ⚔️ {game.get('name')} | `{status.get('displayClock')}`")
        
        c1, c2, c3, c4 = st.columns(4)
        c1.metric("Live Total (Game)", game_total)
        c2.metric("Live Quarter Total", q_total)
        c3.metric("Mirror Projection", f"{mirror_pred:.1f}")
        c4.metric("1H Projection", f"{fh_proj:.1f}")

        # Variance Table
        st.markdown("#### 📊 Multi-Platform Variance Matrix")
        platforms = {
            "📈 Polymarket": {"total": 53.5, "odds": "-110"},
            "🏛️ Kalshi": {"total": 54.0, "odds": "-105"},
            "👑 DraftKings": {"total": 54.5, "odds": "-115"}
        }

        comparison_data = []
        for name, data in platforms.items():
            var = mirror_pred - data['total']
            comparison_data.append({
                "Platform": name,
                "Live Market Total": data['total'],
                "Live Odds": data['odds'],
                "Variance": f"{var:.1f}",
                "Signal": "📈 OVER" if var > 1.5 else ("📉 UNDER" if var < -1.5 else "➖")
            })
        st.table(comparison_data)

# 3. RUN THE FRAGMENT
render_live_dashboard()
